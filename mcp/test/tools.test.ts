...
describe('enforcement de empresa en las tools', () => {
  ...
  it('kronos_metadata expone solo el alcance de la key actual', async () => {
    ...
    expect(meta.capabilities.totalTools).toBe(24);
...
describe('superficie de tools — Kronos + Teams', () => {
  it('las tools de Kronos y Teams están expuestas; las únicas de Teams son de lectura', async () => {
    const server = buildServer();
    const client = await connectClient(server);
    ...
    for (const n of names) {
      expect(n.startsWith('kronos_') || n.startsWith('teams_')).toBe(true);
