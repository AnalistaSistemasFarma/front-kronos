...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const getFileIcon = (type: string) => {
    ...
    if (type.includes('excel') || type.includes('spreadsheet')) return <IconFileSpreadsheet size={20} />;
  ...
  const CheckOrCreateFolderAndUpload = async (folderName: string, files: { file: File }[], token: string) => {
    try {
      ...
      if (getResponse.ok) {
        ...
        const createResponse = await fetch(
          `${process.env.NEXT_PUBLIC_MICROSOFTGRAPHUSERROUTE}root:/SAPSEND/TEC/MA:/children`,
          {
            method: 'POST',
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      if (files && files.length > 0) {
        const uploadPromises = files.map(async (fileWrapper) => {
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_MICROSOFTGRAPHUSERROUTE}items/${folderId}:/${fileWrapper.file.name}:/content`,
            {
              method: 'PUT',
              headers: {
                Authorization: `Bearer ${token}`,
