 ...
 * El firmante en turno recibe notificación de tarea vía notifyActivityAssigned.
 * @param {{ requestId: number, subject?: string|null, signerEmails?: string[], currentSignerEmail?: string|null }} params
 */
export async function notifyOrionSignerInvited({
  ...
  const activitiesUrl = buildAppUrl('/process/request-general/assigned-activities');
  ...
  for (const email of unique) {
    ...
    const result = await sendToEmails([email], {
      ...
      body: `Solicitud #${requestId} — ${subject || 'Sin asunto'}. Recibirá una tarea en "Tareas asignadas" cuando sea su turno.`,
      url: activitiesUrl,
