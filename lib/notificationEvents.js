 ...
 * El firmante en turno recibe una sola notificación vía autorización
 * ("Autorizar firma" / "Su turno"); aquí solo se avisa a quienes aún no les toca.
 * @param {{ requestId: number, subject?: string|null, signerEmails?: string[], currentSignerEmail?: string|null, fileId?: string|null }} params
 */
export async function notifyOrionSignerInvited({
  ...
  for (const email of unique) {
    ...
    const result = await sendToEmails([email], {
      ...
      body: `Solicitud #${requestId} — ${subject || 'Sin asunto'}. Recibirá aviso en Autorizaciones cuando sea su turno.`,
      url: viewUrl,
      tag: `orion-signer-${requestId}-${email.toLowerCase()}`,
    });
