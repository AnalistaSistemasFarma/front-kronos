...
async function resolveRequestStakeholderEmails(requestId, { excludeEmail } = {}) {
  ...
  `);
    return [...new Set(result.recordset.map((r) => r.email).filter(Boolean))].filter(
      (e) =>
        !excludeEmail ||
        String(e).trim().toLowerCase() !== String(excludeEmail).trim().toLowerCase()
    );
  });
}
...
/**
 * Nueva solicitud: creador (confirmación) + encargado de proceso + usuarios con tareas.
 */
export async function notifyNewRequest({
  ...
  const creatorViewUrl = buildAppUrl(
    `/process/request-general/view-request?id=${requestId}&from=create-request`
  );

  const activitiesUrl = buildAppUrl(
    `/process/request-general/view-activities?requestId=${requestId}&from=assigned-activities`
  );

  const creator = creatorEmail
    ? String(creatorEmail).trim().toLowerCase()
    : null;

  const processList = [
    ...new Set(
      [...(processEmails || []), processEmail]
        .filter(Boolean)
        .map((e) => String(e).trim().toLowerCase())
        .filter(Boolean)
    ),
  ];
  const taskList = [...new Set((taskEmails || []).filter(Boolean))]
    .map((e) => String(e).trim().toLowerCase())
    .filter((e) => e && !processList.includes(e));

  const results = [];

  if (creator) {
    results.push(
      await sendToEmails([creator], {
        title: 'Solicitud creada · SynerLink',
        body: `Tu solicitud #${requestId} — ${subject || 'Sin asunto'} fue registrada correctamente.`,
        url: creatorViewUrl,
  ...
  if (processList.length === 0 && taskList.length === 0 && !creator) {
    console.warn(
      `[notifyNewRequest] Solicitud #${requestId}: sin destinatarios (creador, encargado ni tareas).`
    );
    return results;
  }

  // Encargados: no duplicar el mismo mensaje si el creador también es encargado
  // (ya recibió confirmación; igual avisarles como “Nueva solicitud” si no son el creador).
  const processNotifyList = processList.filter((e) => e !== creator);
  if (processNotifyList.length > 0) {
    results.push(
      await sendToEmails(processNotifyList, {
  ...
  } else if (processList.length > 0 && creator && processList.includes(creator)) {
    // Creador = único encargado: la confirmación ya cubre el aviso en campana.
  }

  const taskNotifyList = taskList.filter((e) => e !== creator);
  if (taskNotifyList.length > 0) {
    results.push(
      await sendToEmails(taskNotifyList, {
...
export async function notifyActivityAssigned({ taskId, userId, requestId, subject, taskName }) {
  ...
  let resolvedRequestId = requestId != null && Number(requestId) > 0 ? Number(requestId) : null;
  ...
  return sendToEmails([email], {
    ...
    url: Number(taskId) > 0
      ? buildAppUrl(
          `/process/request-general/view-activities?id=${Number(taskId)}&from=assigned-activities`
        )
      : resolvedRequestId
        ? buildAppUrl(
            `/process/request-general/view-activities?requestId=${resolvedRequestId}&from=assigned-activities`
          )
...
/**
 * Aviso a firmantes Orion cuando el documento entra en flujo de firma.
 * El firmante en turno recibe "Le toca firmar: {archivo}"; el resto,
 * "Incluido como firmante". También se crea la auth en Autorizaciones.
 * @param {{ requestId: number, subject?: string|null, signerEmails?: string[], currentSignerEmail?: string|null, fileId?: string|null, fileName?: string|null }} params
 */
export async function notifyOrionSignerInvited({
  requestId,
  subject,
  signerEmails,
  currentSignerEmail,
  ...
  const emails = Array.isArray(signerEmails) ? signerEmails : [];
  const qs = new URLSearchParams({
    id: String(requestId),
    from: 'general-requests',
  });
  if (fileId) qs.set('orionFileId', String(fileId));
  const viewUrl = buildAppUrl(`/process/request-general/view-request?${qs.toString()}`);
  const current = String(currentSignerEmail || '')
    .trim()
    .toLowerCase();
  const unique = [...new Set(emails.map((e) => String(e || '').trim()).filter(Boolean))];
  const docLabel = String(fileName || '').trim();
  const subjectLabel = String(subject || '').trim() || 'Sin asunto';

  let saved = 0;
  let pushed = 0;

  // Firmante en turno: aviso explícito con nombre del PDF (además de Autorizaciones).
  if (current) {
    const turnQs = new URLSearchParams({ from: 'authorization' });
    if (fileId) turnQs.set('orionFileId', String(fileId));
    const turnResult = await sendToEmails([current], {
      title: docLabel ? `Le toca firmar: ${docLabel}` : 'Le toca firmar · SynerLink',
      body: [
        `Solicitud #${requestId}`,
        docLabel ? `Documento: ${docLabel}` : null,
        subjectLabel,
        'Abra Autorizaciones, autorice y firme este archivo.',
      ]
        .filter(Boolean)
        .join(' · '),
      url: buildAppUrl(`/process/authorization?${turnQs.toString()}`),
      tag: `orion-turn-${requestId}-${fileId || 'doc'}-${current}`,
    });
    saved += turnResult.saved ?? 0;
    pushed += turnResult.pushed ?? 0;
  }

  for (const email of unique) {
    if (email.toLowerCase() === current) continue;

    const result = await sendToEmails([email], {
      title: docLabel
        ? `Incluido como firmante · ${docLabel}`
        : 'Incluido como firmante · SynerLink',
      body: [
        `Solicitud #${requestId}`,
        docLabel ? `Documento: ${docLabel}` : null,
        subjectLabel,
        'Recibirá aviso en Autorizaciones cuando sea su turno.',
      ]
        .filter(Boolean)
        .join(' · '),
      url: viewUrl,
      tag: `orion-signer-${requestId}-${fileId || 'doc'}-${email.toLowerCase()}`,
    });
...
/**
 * Progreso del flujo GSS Firma → campana + push al solicitante y encargados del proceso.
 * @param {{
 *   requestId: number,
 *   subject?: string|null,
 *   fileName?: string|null,
 *   fileId?: string|null,
 *   event: 'sent'|'signer_completed'|'turn'|'all_signed'|'rejected'|'returned',
 *   signerLabel?: string|null,
 *   positionLabel?: string|null,
 *   nextSignerEmail?: string|null,
 *   excludeEmail?: string|null,
 *   excludeEmails?: string[]|null,
 * }} params
 */
export async function notifyOrionSignatureProgress({
  requestId,
  subject,
  fileName,
  fileId,
  ...
  if (!requestId) return { saved: 0, pushed: 0 };

  const excludeSet = new Set(
    [excludeEmail, ...(Array.isArray(excludeEmails) ? excludeEmails : [])]
      .map((e) => String(e || '').trim().toLowerCase())
      .filter(Boolean)
  );

  const emails = (await resolveRequestStakeholderEmails(requestId, {})).filter(
    (e) => !excludeSet.has(String(e || '').trim().toLowerCase())
  );

  if (emails.length === 0) return { saved: 0, pushed: 0 };

  const docPart = fileName ? ` · ${fileName}` : '';
  ...
  if (fid) qs.set('orionFileId', fid);
  const viewUrl = buildAppUrl(`/process/request-general/view-request?${qs.toString()}`);

  let title = 'Firma digital · SynerLink';
  let body = `Solicitud #${requestId}${docPart}${subjectPart}`;
  let tag = `orion-progress-${requestId}-${event}-${Date.now()}`;

  switch (event) {
    case 'sent':
      title = 'Documento enviado a firma · SynerLink';
      body = `Solicitud #${requestId}${docPart}${subjectPart}. El flujo de firma comenzó.`;
      tag = `orion-sent-${requestId}-${fileName || 'doc'}`;
      break;
    case 'signer_completed':
      title = 'Firma avanzada · SynerLink';
      body = `Solicitud #${requestId}${docPart}: ${
        positionLabel || 'un firmante'
      }${signerLabel ? ` (${signerLabel})` : ''} completó su firma.`;
      tag = `orion-signed-${requestId}-${positionLabel || 'x'}-${Date.now()}`;
      break;
    case 'turn':
      title = 'Siguiente firmante · SynerLink';
      body = `Solicitud #${requestId}${docPart}: turno de ${
        nextSignerEmail || 'siguiente firmante'
      }.`;
      tag = `orion-turn-${requestId}-${String(nextSignerEmail || 'next').toLowerCase()}`;
      break;
    case 'all_signed':
      title = 'Documento firmado por completo · SynerLink';
      body = `Solicitud #${requestId}${docPart}${subjectPart}. Todos los firmantes completaron la firma.`;
      tag = `orion-complete-${requestId}-${fileName || 'doc'}`;
      break;
    case 'rejected':
      title = 'Documento rechazado · SynerLink';
      body = `Solicitud #${requestId}${docPart}${subjectPart}. Un firmante rechazó el documento.`;
      tag = `orion-rejected-${requestId}`;
      break;
    case 'returned':
      title = 'Documento devuelto · SynerLink';
      body = `Solicitud #${requestId}${docPart}${subjectPart}. Devuelto para corrección.`;
      tag = `orion-returned-${requestId}-${Date.now()}`;
      break;
    default:
      break;
  }

  return sendToEmails(emails, { title, body, url: viewUrl, tag });
}
