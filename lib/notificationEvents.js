...
export async function notifyOrionSignatureProgress({
  ...
  const exclude = String(excludeEmail || '')
    .trim()
    .toLowerCase();
  const emails = (
    await resolveRequestStakeholderEmails(requestId, {
      excludeEmail: exclude || undefined,
    })
  ).filter((e) => String(e || '').trim().toLowerCase() !== exclude);
