 ...
 * @returns {Promise<{ id_request: number, processEmail: string|null, processEmails: string[], taskEmails: string[] }>}
 */
export async function createGeneralRequest({
  ...
  try {
    ...
    const taskEmails = [
      ...new Set(
        createdRows
          .map((t) => String(t.email || '').trim().toLowerCase())
          .filter(Boolean)
      ),
    ];
    ...
    return { id_request: newRequestId, processEmail, processEmails, taskEmails };
  } catch (dbError) {
