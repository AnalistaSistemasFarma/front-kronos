...
/**
 * Filtro del dropdown "Técnico asignado" (app/api/help-desk/tickets/route.js).
 *
 * El dropdown envía UN solo id_subprocess_user_company (@technician), pero una
 * misma PERSONA puede tener VARIAS filas en subprocess_user_company (una por
 * empresa y/o subproceso). Sus casos (c.id_technical) pueden quedar repartidos
 * entre cualquiera de esas filas, así que filtrar por un único id perdería casos
 * (el técnico aparecía con 0 casos aunque tuviera tickets).
 *
 * Solución: resolver la persona (id_user) dueña de @technician y traer los casos
 * de TODOS sus id_subprocess_user_company. @technician sigue siendo sql.Int.
 */
export const TECHNICIAN_PERSON_FILTER_SQL = `
  c.id_technical IN (
    SELECT sa.id_subprocess_user_company
    FROM subprocess_user_company sa
    INNER JOIN company_user ca ON ca.id_company_user = sa.id_company_user
    WHERE ca.id_user = (
      SELECT cs.id_user
      FROM subprocess_user_company ss
      INNER JOIN company_user cs ON cs.id_company_user = ss.id_company_user
      WHERE ss.id_subprocess_user_company = @technician
    )
  )
`;

/**
 * Valor del filtro "Sin asignar" en el tablero.
 * Debe coincidir EXACTAMENTE con lo que la UI muestra como "Sin asignar"
 * (`nombreTecnico` vacío): no hay persona de técnico resoluble.
 *
 * Se usa NOT EXISTS (no depende de los LEFT JOIN del SELECT) para evitar
 * falsos positivos por JOINs rotos/duplicados que metían casos ya asignados.
 */
export const TECHNICIAN_UNASSIGNED_FILTER_SQL = `
  (
    c.id_technical IS NULL
    OR c.id_technical = 0
    OR NOT EXISTS (
      SELECT 1
      FROM subprocess_user_company suc_u
      INNER JOIN company_user cu_u ON cu_u.id_company_user = suc_u.id_company_user
      INNER JOIN [user] u_u ON u_u.id = cu_u.id_user
      WHERE suc_u.id_subprocess_user_company = c.id_technical
        AND NULLIF(LTRIM(RTRIM(u_u.name)), '') IS NOT NULL
    )
  )
`;

/** Valor sentinela del query param technician=unassigned */
export const TECHNICIAN_UNASSIGNED_VALUE = 'unassigned';

/**
 * ¿El caso se muestra como "Sin asignar" en UI? (sin nombre de técnico)
 */
export function isTechnicianUnassignedDisplay(nombreTecnico) {
  return !nombreTecnico || !String(nombreTecnico).trim();
}

/**
 * Normaliza el id de técnico para INSERT/UPDATE.
 * '' / null / undefined / 0 / 'unassigned' → null (sin asignar).
 */
export function normalizeHelpDeskTechnicianId(value) {
  if (value == null || value === '' || value === TECHNICIAN_UNASSIGNED_VALUE) {
    return null;
  }
