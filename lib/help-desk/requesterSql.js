...
export const REQUESTER_EMAIL_FILTER_SQL = `
  (
    LOWER(LTRIM(RTRIM(${CASE_CONTACT_EMAIL_SQL}))) = LOWER(LTRIM(RTRIM(@requester_email)))
    OR LOWER(LTRIM(RTRIM(${REQUESTER_EMAIL_SQL}))) = LOWER(LTRIM(RTRIM(@requester_email)))
    OR EXISTS (
      SELECT 1 FROM [user] ux
...
export const MY_TICKETS_SCOPE_SQL = `
  (
    (
      LOWER(LTRIM(RTRIM(${CASE_CONTACT_EMAIL_SQL}))) = LOWER(LTRIM(RTRIM(@user_email)))
      OR LOWER(LTRIM(RTRIM(${REQUESTER_EMAIL_SQL}))) = LOWER(LTRIM(RTRIM(@user_email)))
      OR EXISTS (
        SELECT 1 FROM [user] ux
        WHERE LOWER(LTRIM(RTRIM(ux.email))) = LOWER(LTRIM(RTRIM(@user_email)))
          AND LTRIM(RTRIM(ux.name)) != ''
          AND (
            ${REQUESTER_NAME_SQL} LIKE '%' + ux.name + '%'
            OR ux.name LIKE '%' + ${REQUESTER_NAME_SQL} + '%'
