...
describe('configuración de Balances', () => {
  it('habilita Farmalogica, OLP, GSS y Abamia', () => {
    expect(getEnabledBalanceCompanies().map((company) => company.idCompany)).toEqual([1, 3, 8, 6, 7, 9]);
  ...
  it('tiene OLP y GSS configuradas y habilitadas', () => {
    ...
    expect(getBalanceCompany(6)).toMatchObject({ enabled: true, slug: 'meditrack' });
    expect(getBalanceCompany(9)).toMatchObject({ enabled: true, slug: 'kelab' });
    expect(getBalanceCompany(7)).toMatchObject({
      enabled: true,
      slug: 'abamia',
      balanceSqlFile: 'abamia-balance.sql',
      acumuladoSqlFile: 'abamia-acumulado.sql',
    });
    expect(BALANCE_COMPANIES).toHaveLength(6);
  });
