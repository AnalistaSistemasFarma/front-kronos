-- Extraido verbatim del paso 'Balance_Acumulado_OLP_2026' del SQL Server Agent Job compartido
...
DELETE FROM [dbo].[OLP_Balance_Acumulado_2026]

INSERT INTO [dbo].[OLP_Balance_Acumulado_2026] ([Cuenta],[Nombre],[Mes],[Valor])
...
FROM            (SELECT        CASE WHEN T2.levels = 5 THEN T2.AcctCode ELSE T2.AcctCode END AS Cuenta_5,
                          ...
                          FROM     [ONELATAMPHARMA_PROD].DBO.OJDT AS T0 INNER JOIN
                                   [ONELATAMPHARMA_PROD].DBO.JDT1 AS T1 ON T0.TransId = T1.TransId INNER JOIN
                                       (SELECT        AcctCode, FatherNum, Levels
                                        FROM            [ONELATAMPHARMA_PROD].DBO.OACT AS TA1) AS T2 ON T2.AcctCode = T1.Account LEFT OUTER JOIN
                                       (SELECT        AcctCode, FatherNum, Levels
                                        FROM            [ONELATAMPHARMA_PROD].DBO.OACT AS TA2) AS T3 ON T3.AcctCode = T2.FatherNum LEFT OUTER JOIN
                                       (SELECT        AcctCode, FatherNum, Levels
                                        FROM            [ONELATAMPHARMA_PROD].DBO.OACT AS TA3) AS T4 ON T4.AcctCode = T3.FatherNum LEFT OUTER JOIN
                                       (SELECT        AcctCode, FatherNum, Levels
                                        FROM            [ONELATAMPHARMA_PROD].DBO.OACT AS TA4) AS T5 ON T5.AcctCode = T4.FatherNum LEFT OUTER JOIN
									   (SELECT        AcctCode, FatherNum, Levels
                                        FROM            [ONELATAMPHARMA_PROD].DBO.OACT AS TA5) AS T6 ON T6.AcctCode = T5.FatherNum
                         ...
                         GROUP BY T0.TransType, T2.FatherNum, T3.FatherNum, T2.Levels, T3.Levels, 
                             ...
                             (SELECT        AcctCode, AcctName
                               FROM            [ONELATAMPHARMA_PROD].DBO.OACT AS TA5) AS T72 ON T72.AcctCode = T9.Cuenta_5
...
INSERT 
INTO [dbo].[OLP_Balance_Acumulado_2026]([Cuenta], [Nombre], [Mes], [Valor]) 
SELECT T1.[Account], T2.[AcctName],'ENERO' as Mes, Sum(T1.[Debit])- Sum(T1.[Credit]) as Valor
FROM [ONELATAMPHARMA_PROD].DBO.OJDT T0  INNER JOIN [ONELATAMPHARMA_PROD].DBO.JDT1 T1 ON T0.[TransId] = T1.[TransId] 
			  INNER JOIN [ONELATAMPHARMA_PROD].DBO.OACT T2 ON T1.[Account] = T2.[AcctCode] 
			  INNER JOIN [ONELATAMPHARMA_PROD].DBO.OCRD T3 ON T1.U_HBT_Tercero = T3.CardCode 
