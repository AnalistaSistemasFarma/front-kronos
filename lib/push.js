  ...
  await Promise.all(
    subs.map(async (sub) => {
      try {
        ...
        const vapidMismatch =
          statusCode === 401 && /vapid.*mismatch|public key mismatch/i.test(body);
        if (statusCode === 404 || statusCode === 410 || vapidMismatch) {
          // Vencida, revocada, o registrada con otro par VAPID (clave rotada).
          console.warn(
            `[sendPushNotification] Suscripción inválida (${statusCode}${
              vapidMismatch ? ' VAPID mismatch' : ''
            }) para ${email} (sub ${sub.id}). Se elimina.`
          );
