...
export function buildViewTasksQuery(filters: ViewTasksFilters): string {
  ...
  if (filters.task_status) {
    query += ` AND ${ESTADO_TAREA_SQL} = @task_status`;
  }
  if (filters.request_status) {
    query += ` AND scrg.status = @request_status`;
  }
  if (filters.assigned_user) {
    query += ` AND u.name LIKE '%' + @assigned_user + '%'`;
  }
  if (filters.creator) {
    query += ` AND urq.name LIKE '%' + @creator + '%'`;
  }
  if (filters.executor) {
    query += ` AND (uex_t.name LIKE '%' + @executor + '%' OR uex_rg.name LIKE '%' + @executor + '%')`;
  }
  if (filters.company) {
    query += ` AND c.company = @company`;
  }
  if (filters.process) {
    query += ` AND pc.process = @process`;
  }
  if (filters.category) {
    query += ` AND cr.category = @category`;
  }
