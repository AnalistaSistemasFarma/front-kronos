    ...
    .query(`
      ...
      WHERE rg.id = @id
        ...
        AND EXISTS (
          ...
          WHERE rfv.id_request_general = rg.id AND rfv.id_form_field = 58
        )
    `);
...
// Normaliza el "Valor a Pagar": quita separadores de miles (puntos/espacios), toma la coma como
// decimal y trunca a entero. "400.000" → 400000, "400000" → 400000, "400.000,50" → 400000.
...
async function buildTreasuryPayload(pool, requestId) {
  const result = await pool
    ...
    .query(`
      SELECT
        ...
        MAX(CASE WHEN rfv.id_form_field = 58 THEN o.id_request_type END) AS id_request_type,
        MAX(CASE WHEN rfv.id_form_field = 59 THEN o.id_request_subtype END) AS id_request_subtype,
        MAX(CASE WHEN rfv.id_form_field = 60 THEN COALESCE(o.option_label, rfv.value_text) END) AS valor_a_pagar,
        MAX(CASE WHEN rfv.id_form_field = 61 THEN COALESCE(o.option_label, rfv.value_text) END) AS fecha_necesaria,
        MAX(CASE WHEN rfv.id_form_field = 62 THEN COALESCE(o.option_label, rfv.value_text) END) AS concepto,
        MAX(CASE WHEN rfv.id_form_field = 63 THEN COALESCE(o.option_label, rfv.value_text) END) AS acreedor
      FROM requests_general rg
  ...
  return {
    ...
    'Valor a Pagar': parseAmount(row.valor_a_pagar),
