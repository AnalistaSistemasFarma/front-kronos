...
export async function closedSapsendRequest(pool, { id_request }) {
  // a) Datos de la solicitud.
  const reqRes = await pool
    .request()
    .input('id_request', sql.Int, id_request)
  ...
  await pool
    ...
    .query(`
      UPDATE requests_general
      SET status_req = 2,
          resolution = 'Solicitud Cerrada Exitosamente',
          id_executor_final = @executor,
          date_resolution = GETDATE()
      WHERE id = @id_request
    `);
  ...
  const tasksRes = await pool
    ...
    `);
  const tasksCancelled = tasksRes.rowsAffected?.[0] ?? 0;
  console.log(`${TAG} 🚫 Solicitud ${id_request} cerrada (status_req=2); ${tasksCancelled} tarea(s) abiertas canceladas.`);

  // f) Nota de cancelación.
  await insertNote(pool, { id_request, note: 'Solicitud Cerrada Exitosamente', author });
