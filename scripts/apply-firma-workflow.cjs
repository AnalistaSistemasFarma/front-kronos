...
(async () => {
  ...
  const sqlText = fs.readFileSync(
    path.join(root, 'prisma', 'seeds', 'firma-proceso-84-workflow.sql'),
    'utf8'
  );
  await pool.request().query(sqlText);
  const check = await pool.request().input('pid', sql.Int, 84).query(`
    SELECT id, task, display_order, is_sequential, is_authorization, active
    FROM task_process_category
    WHERE id_process_category = @pid
    ORDER BY display_order, id
  `);
  console.log('Tareas proceso 84 tras migración:');
