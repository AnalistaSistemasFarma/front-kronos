/**
 * Sembrar subproceso “Eliminar adjuntos”
 * URL: /process/request-general/delete-attachments
 *
 * Uso:
 *   node scripts/seed-delete-attachments-subprocess.cjs
 *   node scripts/seed-delete-attachments-subprocess.cjs --email=usuario@empresa.com
 */
const fs = require('fs');
const path = require('path');

const URL = '/process/request-general/delete-attachments';
const NAME = 'Eliminar adjuntos';
...
async function main() {
  ...
  try {
    ...
    if (email) {
      ...
      console.log(
        'Subproceso listo. Asígnalo en Administración → Usuarios (o pasa --email=...).'
      );
    }
  } finally {
