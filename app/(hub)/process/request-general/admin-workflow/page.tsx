...
function RequestBoard() {
  ...
  const fetchWorkflowData = async (companyId?: string) => {
    try {
      ...
      if (data.categories && data.categories.length > 0) {
        ...
        setCategoriesWF(
          data.categories
            .filter((c: { id_category: number; category: string }) => {
              const key = c.id_category.toString();
              if (seen.has(key)) return false;
              seen.add(key);
              return true;
            })
            .map((c: { id_category: number; category: string }) => ({
  ...
  const fetchWorkFlowsWithUserId = async (userIdToUse: number, filtersToUse?: typeof filters) => {
    try {
      ...
      const url = `/api/requests-general/workflow-administration?${params.toString()}`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.categories && Array.isArray(data.categories)) {
          ...
          setCategoriesWF(
            data.categories
              .filter((sub: { id_category: number; category: string }) => {
                const key = sub.id_category.toString();
                if (seen.has(key)) return false;
                seen.add(key);
                return true;
              })
              .map((sub: { id_category: number; category: string }) => ({
      ...
      <div className='max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8'>
        <Card shadow='sm' p='xl' radius='md' withBorder mb='6'>
          ...
          <Grid>
            ...
            <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
              <Card p='md' radius='md' withBorder style={{ backgroundColor: 'var(--mantine-color-default)' }}>
                <Group>
