...
import { cancelSapsendRequest } from '../../../../lib/sapsend/resolveWorkflowTask.js';
import {
...
export async function POST(req) {
  try {
    ...
    const outcome = await withMssqlPool((pool) => cancelSapsendRequest(pool, { id_request }));

    if (outcome.notFound) {
      return NextResponse.json({ error: 'Solicitud no encontrada' }, { status: 404 });
    }

    // Notificación de cancelación al solicitante (solo en la transición a cancelada).
