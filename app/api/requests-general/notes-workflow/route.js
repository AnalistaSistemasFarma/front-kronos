  ...
  try {
    ...
    if (!id_process_category) {
      return NextResponse.json(
        { error: 'El id_process_category es requerido' },
        { status: 400 }
      );
    }
    ...
    request.input('id_process_category', sql.Int, id_process_category);
...
export async function POST(req) {
  try {
    ...
    const { id_process_category, note, created_by } = body;

    if (!id_process_category || !note || note.trim() === '') {
      return new Response(
        JSON.stringify({
          error: 'Campos obligatorios faltantes',
          details: 'El id_process_category y el contenido de la nota son requeridos',
        }),
        { status: 400 }
      );
    }
    ...
    try {
      ...
      const insertNoteQuery = `
        INSERT INTO notes (note, id_process_category, created_by)
        OUTPUT INSERTED.id_note
        VALUES (@note, @id_process_category, @created_by);
      `;
